...
exports.fullSUM = function(num) {
  return (
    Math.trunc(num)
      .toString()
      .replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1 ") + "sum"
  );
};

exports.shortSUM = function(num) {
  console.log("shortSUM: ", num);
  let result;
  if (num > 10000000) {
    result =
      Math.trunc(num / 1000000)
        .toString()
        .replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1 ") + "mln sum";
  } else {
    if (num > 10000) {
      result =
        Math.round(num / 1000)
          .toString()
          .replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1 ") + "k sum";
    } else {
      result =
        Math.trunc(num)
          .toString()
...
exports.shortSAT = function(num) {
  ...
  if (num > 10000000) {
    result =
      Math.trunc(num / 1000000)
        .toString()
        .replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1 ") + "mln sat";
  } else {
    if (num > 100000) {
      result =
        Math.round(num / 1000)
          .toString()
          .replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1 ") + "k sat";
    } else {
      result =
        Math.trunc(num)
          .toString()
          .replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1 ") + "sat";
    }
  }
  return result;
...
exports.createInvoice = function(qty_sum, qty_btc, contract_id) {
  ...
  return {
    ...
    title: "Биткоин (BTC)",
    description: `${this.shortSAT(qty_btc * 100000000)} (${this.fullBTC(
      qty_btc
    )})`,
    currency: "UZS",
    is_flexible: false,
    ...
    payload: contract_id,
    photo_url:
      "https://cdn.paycom.uz/merchants/8b46bb4b52e4e8e7f396ad673165f66ca85ad5e2.png",
